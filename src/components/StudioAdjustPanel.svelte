<script>
  ...
  const cropLabels = { free:'Libre', '16:9':'16:9', '4:3':'4:3', '1:1':'1:1', '3:2':'3:2' };
...
{#snippet stSlider(label, value, initialValue, min, max, step, onChange, fixed = 2, suffix = '')}
  ...
  <div class="sa-slider-row" class:modified={value !== initialValue}
       onwheel={(e) => {
         e.preventDefault();
...
<div class="sa-layout">
  <div class="sa-panel" class:open={isOpen}>
    <!-- Trigger -->
    <div class="sa-trigger" onclick={toggleOpen} role="button" tabindex="0" onkeydown={(e) => e.key === 'Enter' && toggleOpen()}>
      ...
      <div class="sa-trigger-text">
        ...
        {#if !isOpen}
          ...
          <span class="sa-trigger-dim">{studioState.brightness.toFixed(1)}b</span>
    ...
    <div class="sa-content">
      <div class="sa-scroll">
        ...
        <section class="sa-section">
          ...
          {@render stSlider('Luminosité', studioState.brightness, STUDIO.brightnessDefault, STUDIO.brightnessMin, STUDIO.brightnessMax, STUDIO.brightnessStep, (e) => studioState.brightness = +e.target.value)}
          {@render stSlider('Contraste', studioState.contrast, STUDIO.contrastDefault, STUDIO.contrastMin, STUDIO.contrastMax, STUDIO.contrastStep, (e) => studioState.contrast = +e.target.value)}
          {@render stSlider('Clarté', studioState.clarity, STUDIO.clarityDefault, STUDIO.clarityMin, STUDIO.clarityMax, STUDIO.clarityStep, (e) => studioState.clarity = +e.target.value)}
        ...
        <section class="sa-section">
          ...
          {@render stSlider('Intensité', studioState.vignette, STUDIO.vignetteDefault, STUDIO.vignetteMin, STUDIO.vignetteMax, STUDIO.vignetteStep, (e) => studioState.vignette = +e.target.value)}
          {@render stSlider('Douceur', studioState.vignetteFeather, STUDIO.vignetteFeatherDefault, STUDIO.vignetteFeatherMin, STUDIO.vignetteFeatherMax, STUDIO.vignetteFeatherStep, (e) => studioState.vignetteFeather = +e.target.value)}
        </section>
...
<style>
  ...
  .sa-panel.open .sa-trigger-icon { transform: rotate(90deg); }
  ...
  .sa-trigger-sep { opacity: 0.3; }
  .sa-trigger-dim { opacity: 0.6; }
  ...
  .sa-panel.open .sa-content { opacity: 1; pointer-events: auto; transition: opacity 0.5s ease 0.1s; }
  ...
  .sa-div {
    height: 1px; background: rgba(255, 64, 129, 0.08); margin: 4px 0;
  }
  ...
  @keyframes sa-pulse-glow {
    from { opacity: 0.7; box-shadow: 0 0 2px #FF4081; }
    to { opacity: 1; box-shadow: 0 0 6px #FF4081; }
  }

  .sa-slider-label {
    font-size: 11px; font-weight: 500; color: var(--color-text-dim);
    min-width: 58px; max-width: 62px;
  ...
  .sa-slider {
    ...
    height: 4px; background: rgba(255, 64, 129, 0.15);
  ...
  .sa-slider::-webkit-slider-thumb {
    ...
    background: #FF4081; border: 2px solid rgba(255, 255, 255, 0.3);
    cursor: pointer; box-shadow: 0 0 6px rgba(255, 64, 129, 0.5);
  ...
  .sa-slider-val-wrapper {
    ...
    width: 38px; justify-content: flex-end; gap: 2px;
  }

  .sa-number-input {
    background: transparent; border: none; color: #FF4081;
    font-family: var(--font-mono); font-size: 10px; width: 100%;
    text-align: right; padding: 0; margin: 0; outline: none; line-height: 1;
  ...
  .sa-number-input:focus {
    color: #fff; background: rgba(255, 64, 129, 0.2); border-radius: 2px;
  }

  .sa-suffix {
    color: #FF4081; font-family: var(--font-mono); font-size: 10px;
  }

  /* Toggle */
  .sa-toggle {
    display: flex; align-items: center; gap: 8px; cursor: pointer; margin: 6px 0;
  }
  ...
  .sa-ttrack {
    ...
    border-radius: 999px; transition: all 0.3s; flex-shrink: 0;
  }

  .sa-tthumb {
    position: absolute; top: 1px; left: 1px;
    width: 10px; height: 10px; border-radius: 50%;
    background: var(--color-text-dim); transition: all 0.3s;
  }

  .sa-toggle input:checked + .sa-ttrack {
    background: rgba(255, 64, 129, 0.2); border-color: rgba(255, 64, 129, 0.4);
  }

  .sa-toggle input:checked + .sa-ttrack .sa-tthumb {
    transform: translateX(14px); background: #FF4081;
    box-shadow: 0 0 6px rgba(255, 64, 129, 0.5);
  }

  .sa-toggle-label { font-size: 11px; color: var(--color-text-dim); }

  /* Flip buttons */
  .sa-flip-row { display: flex; gap: 6px; margin: 6px 0; }
  ...
  .sa-flip {
    ...
    font-family: var(--font-main); font-size: 10px; font-weight: 500;
    cursor: pointer; transition: all var(--transition-med);
  }

  .sa-flip svg { width: 16px; height: 16px; flex-shrink: 0; }
  ...
  /* Crop pills */
  .sa-crop-row { display: flex; align-items: center; gap: 8px; margin: 6px 0; }

  .sa-crop-pills { display: flex; gap: 4px; flex-wrap: wrap; }
  ...
  .sa-pill:hover { border-color: rgba(255, 64, 129, 0.4); background: rgba(255, 64, 129, 0.08); }

  .sa-pill.active {
    background: #FF4081; color: #fff; border-color: #FF4081;
    box-shadow: 0 0 8px rgba(255, 64, 129, 0.4); font-weight: 600;
  }
  ...
  .sa-search {
    ...
    border-radius: var(--radius-sm);
    color: var(--color-text); font-family: var(--font-main); font-size: 11px;
    outline: none; transition: border-color var(--transition-fast);
    margin-bottom: 6px;
  }

  .sa-search:focus { border-color: rgba(255, 64, 129, 0.4); }

  .sa-search::placeholder { color: var(--color-text-dim); opacity: 0.5; }

  .sa-crater-list {
    max-height: 120px; overflow-y: auto;
    scrollbar-width: none; margin-bottom: 8px;
  }
  .sa-crater-list::-webkit-scrollbar { display: none; }

  .sa-crater-empty {
    font-size: 10px; color: var(--color-text-dim); opacity: 0.4;
    text-align: center; padding: 12px 0;
  }

  /* Footer */
  .sa-footer { flex-shrink: 0; }
  ...
  .sa-reset-btn svg { width: 14px; height: 14px; }
